<template>
  ...
  <div v-show="treeView.showTree" class="c-dev">
    ...
    <ButtonDevion
      :label="changeDev.label"
      :isDisabled="changeDev.isDisabled"
      :showButton="changeDev.showButton"
      @click="() => TogglePopup('changeDevion')"
      class="c-button c-change-dev"
    />
  </div>
  <labelDevion :label="missing.label" :showLabel="missing.showlabel" class="c-artikel-missing" />
  <PopupDevion
    :TogglePopup="() => TogglePopup('changeDevion')"
    :data="changeDev.data"
    v-show="popupTriggers.changeDevion"
  ...
  <div v-show="treeViewMet.showTree" class="c-metabil">
    ...
    <buttonDevion
      :label="changeMet.label"
      :isDisabled="changeMet.isDisabled"
      :showButton="changeMet.showButton"
      @click="() => TogglePopup('changeMetabil')"
      class="c-button c-change-met"
    />
  </div>
  <PopupDevion
    :TogglePopup="() => TogglePopup('changeMetabil')"
    :data="changeMet.data"
    v-show="popupTriggers.changeMetabil"
    @change-items="updateItemsMet"
  />
  <labelDevion
    :label="missingMet.label"
    :showLabel="missingMet.showlabel && LinkModel"
    class="c-artikel-missing"
  />
  ...
  <div v-if="treeView.showTree || treeViewMet.showTree" class="c-buttons">
    <ButtonDevion
      ...
      :isDisabled="buttonSave.isDisabled"
      :showButton="buttonSave.showButton"
      @click="handleButtonSave"
      class="c-button c-button-tree"
    />
    <ButtonDevion
      :label="buttonInsert.label"
      :isDisabled="buttonInsert.isDisabled"
      :showButton="buttonInsert.showButton"
  ...
  <div class="c-artikel-form">
    <artikelForm
      :showform="artikelForm.showform"
      :data="artikelForm.data"
      @object-artikel="handleArtikel"
      :priceData="artikelForm.priceData"
      class="c-form"
      :check="artikelForm.check"
      ref="article"
    />
    <div class="c-artikel-button--save" :class="{ 'o-hide-accessible': !save.showButton }">
      <buttonDevion
        :label="save.label"
        :isDisabled="save.isButtonDisabled"
        :showButton="save.showButton"
        @click="handleSaveButtonClick"
        class="c-button-artikel--next"
      />
    </div>
    <div class="c-artikel-button--next" :class="{ 'o-hide-accessible': !next.showButton }">
      <buttonDevion
        :label="next.label"
        :isDisabled="next.isButtonDisabled"
        :showButton="next.showButton"
    ...
    <div class="c-artikel-button--prev" :class="{ 'o-hide-accessible': !prev.showButton }">
      <buttonDevion
        :label="prev.label"
        :isDisabled="prev.isButtonDisabled"
        :showButton="prev.showButton"
        @click="handlePrevButtonClick"
        class="c-button-artikel--prev"
      />
    </div>
    <div class="c-artikel-progress">
      <labelDevion
        :label="artikelProgress.label"
        :showLabel="artikelProgress.showLabel"
  ...
  methods: {
    ...
    async checkArtikelMet(artikel) {
      ...
      if (artikel) {
        ...
        GetData('Metabil/ets/articleexists?ArticleNumber=' + artikel.number)
          .then((result) => result)
          .then((data) => {
    ...
    async handleButtonInsert() {
      if (notFound > 0) {
        ...
        this.loading.showLoad = true
        PutDataWithBody('devion/ets/updatelinkedarticles', artikelen)
          .then((response) => {
            this.loading.showLoad = false
        ...
        if (artikelenMet.length > 0 && LinkModel.value) {
          PutDataWithBody('metabil/ets/updatelinkedarticles', artikelenMet)
            .then((response) => {
              this.loading.showLoad = false
              this.linked.showlabel = true
              let log = JSON.parse(response)
